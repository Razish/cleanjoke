...
// The editor only specifies a single value for angles (yaw), but we have special constants to generate an up or down
//	direction.
...
static void G_SpewEntList(void)
{
	...
	str = va("TempEnt count: %i\nTempEnt ST: %i\nProjectile count: %i\n", numTempEnt, numTempEntST, numProjectile);
	Com_Printf(str);
#ifdef _DEBUG
	if (fh)
	{
...
// The origin will be snapped to save net bandwidth, so care must be taken if the origin is right on a surface (snap
//	towards start vector first)
...
void TryUse( gentity_t *ent )
{
	...
	if ( ValidUseTarget( target ) ) {
		...
		ent->client->ps.weaponTime = ent->client->ps.torsoTimer;
		/*
		G_SetAnim( ent, SETANIM_TORSO, BOTH_FORCEPUSH, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
		if ( !VectorLengthSquared( ent->client->ps.velocity ) )
		{
			G_SetAnim( ent, SETANIM_LEGS, BOTH_FORCEPUSH, SETANIM_FLAG_NORMAL|SETANIM_FLAG_HOLD );
		}
		*/
