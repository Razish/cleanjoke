...
void UI_LoadNonIngame() {
	UI_LoadMenus( MENUSET_DEFAULT, false );
	...
	while ( 1 ) {
		...
		if (Q_stricmp(token.string, "precacheSound") == 0)
		{
			...
			if (PC_Script_Parse(handle, &tempStr))
			{
				...
				do
				{
					soundFile = COM_ParseExt(&tempStr, false);
...
void UI_Load( void ) {
	...
	UI_LoadMenus( uiInfo.inGameLoad ? MENUSET_INGAME : MENUSET_DEFAULT, true );
	Menus_CloseAll();
...
static void UI_DrawHandicap(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont) {
	...
	h = Com_Clamp(5, 100, handicap.value);
	i = 20 - h / 5;

	Font font( iMenuFont, scale );
	font.Paint(rect->x, rect->y, handicapValues[i], color, textStyle);
}

static void UI_DrawClanName(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont) {
	Font font( iMenuFont, scale );
	font.Paint(rect->x, rect->y, ui_teamName.string, color, textStyle);
}
...
{
	Font font( iMenuFont, scale );
	font.Paint(rect->x, rect->y, UI_GetGameTypeName(uiInfo.gameTypes[ui_gametype.integer].gtEnum), color, textStyle);
}
...
static void UI_DrawClanLogo(rectDef_t *rect, float scale, vec4_t color) {
	int i;
	i = UI_TeamIndexFromName(ui_teamName.string);
...
static void UI_DrawSkill(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont) {
	...
	Font font( iMenuFont, scale );
	font.Paint(rect->x, rect->y, (char *)UI_GetStringEdString("MP_INGAME", (char *)skillLevels[i-1]), color, textStyle);
}
...
static void UI_DrawForceSide(rectDef_t *rect, float scale, vec4_t color, int textStyle, int val, int min, int max, int iMenuFont)
{
	...
	if (val == FORCE_LIGHTSIDE)
	{
		...
		if (menu)
		{
			Menu_ShowItemByName(menu, "lightpowers", true);
			Menu_ShowItemByName(menu, "darkpowers", false);
			Menu_ShowItemByName(menu, "darkpowers_team", false);

			Menu_ShowItemByName(menu, "lightpowers_team", true);//(ui_gameType.integer >= GT_TEAM));
	...
	else
	{
		...
		if (menu)
		{
			Menu_ShowItemByName(menu, "lightpowers", false);
			Menu_ShowItemByName(menu, "lightpowers_team", false);
			Menu_ShowItemByName(menu, "darkpowers", true);

			Menu_ShowItemByName(menu, "darkpowers_team", true);//(ui_gameType.integer >= GT_TEAM));
		}
		menu = Menus_FindByName("ingame_playerforce");
		if (menu)
		{
...
bool UI_HasSetSaberOnly( const char *info, const int gametype )
{
	int i = 0;
	int wDisable = 0;
...
static bool UI_AllForceDisabled(int force)
{
	int i;
...
bool UI_TrueJediEnabled( void )
{
	...
	bool saberOnly = false, allForceDisabled = false;
...
static void UI_DrawTeamName(rectDef_t *rect, float scale, vec4_t color, bool blue, int textStyle, int iMenuFont) {
	int i;
	i = UI_TeamIndexFromName((blue) ? ui_blueteam.string : ui_redteam.string);
	if (i >= 0 && i < uiInfo.teamCount) {
		Font font( iMenuFont, scale );
		font.Paint(rect->x, rect->y, va("%s: %s", (blue) ? "Blue" : "Red", uiInfo.teamList[i].teamName), color, textStyle);
	}
}

static void UI_DrawTeamMember(rectDef_t *rect, float scale, vec4_t color, bool blue, int num, int textStyle, int iMenuFont)
{
	// 0 - None
	// 1 - Human
	// 2..NumCharacters - Bot
...
void UpdateBotButtons(void)
{
	...
	Menu_ShowItemByName( menu, "humanbotfield", true );
	Menu_ShowItemByName( menu, "humanbotnonfield", false );
}
...
*/
	...
	if (menu)
	{
		char	info[MAX_INFO_STRING];
		int		disabledForce = 0;
		bool trueJedi = false, allForceDisabled = false;
		...
		{
			Menu_ShowItemByName(menu, "jedinonjedi", false);
		}
		else
		{
			Menu_ShowItemByName(menu, "jedinonjedi", true);
		}
		if ( allForceDisabled == true || (trueJedi && uiJediNonJedi == FORCE_NONJEDI) )
		{	// No force stuff
			Menu_ShowItemByName(menu, "noforce", true);
			Menu_ShowItemByName(menu, "yesforce", false);
			// We don't want the saber explanation to say "configure saber attack 1" since we can't.
			Menu_ShowItemByName(menu, "sabernoneconfigme", false);
		}
		else
		{
			UI_SetForceDisabled(disabledForce);
			Menu_ShowItemByName(menu, "noforce", false);
			Menu_ShowItemByName(menu, "yesforce", true);
		}
		...
		{	// Show lightsaber stuff.
			...
			Menu_ShowItemByName(menu, "yessaber", true);
		}
		else
		{
			Menu_ShowItemByName(menu, "nosaber", true);
			Menu_ShowItemByName(menu, "yessaber", false);
		}

		// The leftmost button should be "apply" unless you are in spectator, where you can join any team.
		if ((int)(ui_myteam.value) != TEAM_SPECTATOR)
		{
			Menu_ShowItemByName(menu, "playerapply", true);
			Menu_ShowItemByName(menu, "playerforcejoin", false);
			Menu_ShowItemByName(menu, "playerforcered", true);
			Menu_ShowItemByName(menu, "playerforceblue", true);
			Menu_ShowItemByName(menu, "playerforcespectate", true);
		}
		else
		{
			...
			{	// This is a team-based game.
				...
				{	// Show red or blue based on what side is chosen.
					...
					{
						Menu_ShowItemByName(menu, "playerforcered", false);
						Menu_ShowItemByName(menu, "playerforceblue", true);
					}
					else if (uiForceSide==FORCE_DARKSIDE)
					{
						Menu_ShowItemByName(menu, "playerforcered", true);
						Menu_ShowItemByName(menu, "playerforceblue", false);
					}
					else
					{
						Menu_ShowItemByName(menu, "playerforcered", true);
			...
			Menu_ShowItemByName(menu, "playerapply", false);
			Menu_ShowItemByName(menu, "playerforcejoin", true);
			Menu_ShowItemByName(menu, "playerforcespectate", true);
		}
	}
...
static void UI_DrawNetSource(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont)
{
	...
	Font font( iMenuFont, scale );
	font.Paint(rect->x, rect->y, va("%s %s",holdSPString,
		GetNetSourceString(ui_netSource.integer)), color, textStyle);
}
...
static void UI_DrawNetFilter(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont)
{
	...
	Font font( iMenuFont, scale );
	font.Paint(rect->x, rect->y, va("%s %s",holdSPString, UI_FilterDescription( ui_serverFilterType.integer )), color, textStyle);
}

static void UI_DrawTier(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont) {
	int i;
	i = ui_currentTier.value;
	if (i < 0 || i >= uiInfo.tierCount) {
		i = 0;
	}
	Font font( iMenuFont, scale );
	font.Paint(rect->x, rect->y, va("Tier: %s", uiInfo.tierList[i].tierName), color, textStyle);
}
...
static void UI_DrawTierMapName(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont) {
	...
	j = ui_currentMap.value;
	if (j < 0 || j >= MAPS_PER_TIER) {
		j = 0;
	}

	Font font( iMenuFont, scale );
	font.Paint(rect->x, rect->y, UI_EnglishMapName(uiInfo.tierList[i].maps[j]), color, textStyle);
}
...
static void UI_DrawTierGameType(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont) {
	...
	Font font( iMenuFont, scale );
	font.Paint(rect->x, rect->y, uiInfo.gameTypes[uiInfo.tierList[i].gameTypes[j]].gameType, color, textStyle);
}
...
static void UI_DrawOpponent(rectDef_t *rect) {
  ...
  char team[256];
	...
	if (updateOpponentModel) {
		...
		strcpy(model, ui_opponentModel.string);
	  strcpy(headmodel, ui_opponentModel.string);
		team[0] = '\0';
    ...
    UI_PlayerInfo_SetInfo( &info2, TORSO_WEAPONREADY3, TORSO_WEAPONREADY3, viewangles, vec3_origin, WP_BRYAR_PISTOL, false );
...
static void UI_NextOpponent() {
	int i = UI_TeamIndexFromName(ui_opponentName.string);
	int j = UI_TeamIndexFromName(ui_teamName.string);
	i++;
...
static void	UI_DrawPlayerLogo(rectDef_t *rect, vec3_t color) {
	int i = UI_TeamIndexFromName(ui_teamName.string);
	if (uiInfo.teamList[i].teamIcon == -1) {
		uiInfo.teamList[i].teamIcon = trap->R_RegisterShaderNoMip(uiInfo.teamList[i].imageName);
		uiInfo.teamList[i].teamIcon_Metal = trap->R_RegisterShaderNoMip(va("%s_metal", uiInfo.teamList[i].imageName));
		uiInfo.teamList[i].teamIcon_Name = trap->R_RegisterShaderNoMip(va("%s_name", uiInfo.teamList[i].imageName));
...
static void UI_DrawOpponentName(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont) {
	Font font( iMenuFont, scale );
	font.Paint(rect->x, rect->y, ui_opponentName.string, color, textStyle);
}
...
//			s = "Jedi";
    ...
    case UI_BLUETEAMNAME:
      ...
      break;
    case UI_REDTEAMNAME:
		i = UI_TeamIndexFromName(ui_redteam.string);
		if (i >= 0 && i < uiInfo.teamCount) {
...
static void UI_DrawBotSkill(rectDef_t *rect, float scale, vec4_t color, int textStyle,int iMenuFont)
{
	...
	{
		Font font( iMenuFont, scale );
		font.Paint(rect->x, rect->y, (char *)UI_GetStringEdString("MP_INGAME", (char *)skillLevels[uiInfo.skillIndex]), color, textStyle);
	}
}
...
{
	Font font( iMenuFont, scale );
	font.Paint(rect->x, rect->y, (uiInfo.redBlue == 0) ? UI_GetStringEdString("MP_INGAME","RED") : UI_GetStringEdString("MP_INGAME","BLUE"), color, textStyle);
}
...
static void UI_DrawSelectedPlayer(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont) {
	...
	Font font( iMenuFont, scale );
	font.Paint(rect->x, rect->y, cg_selectedPlayerName.string, color, textStyle);
}
...
static void UI_DrawServerRefreshDate(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont)
{
	...
	if (uiInfo.serverStatus.refreshActive)
	{
		...
		font.Paint(rect->x, rect->y, va((char *) holdSPString, trap->LAN_GetServerCount(UI_SourceForLAN())), newColor, textStyle);
	}
	else
	{
		char buff[64];
...
//		font.Paint(rect->x, rect->y, "Press ENTER or CLICK to change, Press BACKSPACE to clear", color, textStyle);
	}
}
...
static void UI_DrawGLInfo(rectDef_t *rect, float scale, vec4_t color, int textStyle,int iMenuFont)
{
	...
	font.Paint(rect->x + 2, rect->y, va("GL_VENDOR: %s", uiInfo.uiDC.glconfig.vendor_string), color, textStyle, rect->w);
	font.Paint(rect->x + 2, rect->y + 15, va("GL_VERSION: %s: %s", uiInfo.uiDC.glconfig.version_string,uiInfo.uiDC.glconfig.renderer_string), color, textStyle, rect->w);
	font.Paint(rect->x + 2, rect->y + 30, va ("GL_PIXELFORMAT: color(%d-bits) Z(%d-bits) stencil(%d-bits)", uiInfo.uiDC.glconfig.colorBits, uiInfo.uiDC.glconfig.depthBits, uiInfo.uiDC.glconfig.stencilBits), color, textStyle,rect->w);
	...
	while ( y < rect->y + rect->h && extensionName != NULL )
	{
		if ( (i % 2) == 0 )
		{
			font.Paint (rect->x + 2, y, extensionName, color, textStyle, (rect->w / 2));
		}
		else
		{
			font.Paint (rect->x + rect->w / 2, y, extensionName, color, textStyle, (rect->w / 2));
			y += 11;
		}
...
static void UI_Version( rectDef_t *rect, float scale, vec4_t color, int iMenuFont ) {
	Font font( iMenuFont, scale, ITEM_ALIGN_RIGHT );
	font.Paint( rect->x, rect->y, JK_VERSION, color );
}
...
/*
		...
		if (uiForceUsed<3)
		{
		    Menu_ShowItemByName(menu, "lightpowers_team", true);
		}
...
//      UI_DrawTotalForceStars(&rect, scale, color, textStyle);
      break;
    case UI_BLUETEAMNAME:
      UI_DrawTeamName(&rect, scale, color, true, textStyle, iMenuFont);
      break;
    case UI_REDTEAMNAME:
      UI_DrawTeamName(&rect, scale, color, false, textStyle, iMenuFont);
      break;
    case UI_BLUETEAM1:
		case UI_BLUETEAM2:
		case UI_BLUETEAM3:
	...
	else
	{
      ...
      UI_DrawTeamMember(&rect, scale, color, true, iUse, textStyle, iMenuFont);
      break;
    case UI_REDTEAM1:
		case UI_REDTEAM2:
		case UI_REDTEAM3:
	...
	else
	{
      ...
      UI_DrawTeamMember(&rect, scale, color, false, iUse, textStyle, iMenuFont);
      break;
		case UI_NETSOURCE:
    ...
    case UI_NETMAPCINEMATIC:
      ...
      UI_DrawNetFilter(&rect, scale, color, textStyle, iMenuFont);
		...
		case UI_TIER_GAMETYPE:
			...
			break;
		case UI_ALLMAPS_SELECTION:
			UI_DrawAllMapsSelection(&rect, scale, color, textStyle, true, iMenuFont);
			break;
		case UI_MAPS_SELECTION:
			UI_DrawAllMapsSelection(&rect, scale, color, textStyle, false, iMenuFont);
			break;
		case UI_OPPONENT_NAME:
...
static bool UI_OwnerDrawVisible(int flags) {
	...
	while (flags) {
		if (flags & UI_SHOW_FFA) {
			if (g_gametype.value != GT_FFA &&
				g_gametype.value != GT_HOLOCRON &&
				g_gametype.value != GT_JEDIMASTER) {
				vis = false;
			}
			flags &= ~UI_SHOW_FFA;
		}
...
static bool UI_Chat_Main_HandleKey(int key)
{
	menuDef_t *menu;
	itemDef_t *item;
...
// For hot keys on the chat main menu.
static bool UI_Chat_Attack_HandleKey(int key)
{
	menuDef_t *menu;
	itemDef_t *item;
...
// For hot keys on the chat main menu.
static bool UI_Chat_Defend_HandleKey(int key)
{
	menuDef_t *menu;
	itemDef_t *item;
...
// For hot keys on the chat main menu.
static bool UI_Chat_Request_HandleKey(int key)
{
	menuDef_t *menu;
	itemDef_t *item;
...
// For hot keys on the chat main menu.
static bool UI_Chat_Reply_HandleKey(int key)
{
	menuDef_t *menu;
	itemDef_t *item;
...
// For hot keys on the chat main menu.
static bool UI_Chat_Spot_HandleKey(int key)
{
	menuDef_t *menu;
	itemDef_t *item;
...
// For hot keys on the chat main menu.
static bool UI_Chat_Tactical_HandleKey(int key)
{
	menuDef_t *menu;
	itemDef_t *item;
...
static bool UI_AutoSwitch_HandleKey(int flags, float *special, int key) {
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
		int switchVal = cg_autoswitch.value;
...
static bool UI_SelectedPlayer_HandleKey(int flags, float *special, int key) {
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
		...
		selected = cg_selectedPlayer.value;
...
/*
static bool UI_VoiceChat_HandleKey(int flags, float *special, int key)
{
...
static bool UI_OwnerDrawHandleKey(int ownerDraw, int flags, float *special, int key) {
  ...
  switch (ownerDraw) {
	...
	else
	{
      ...
      UI_TeamMember_HandleKey(flags, special, key, true, iUse);
      break;
    case UI_REDTEAM1:
		case UI_REDTEAM2:
		case UI_REDTEAM3:
	...
	else
	{
      ...
      UI_TeamMember_HandleKey(flags, special, key, false, iUse);
      break;
		case UI_NETSOURCE:
...
static void UI_LoadDemosInDirectory( loadDemoContext_t *ctx, const char *directory )
{
	...
	if ( protocolLegacy == com_protocol.integer )
		protocolLegacy = 0;
...
static void UI_Update(const char *cvarName) {
	int	val = trap->Cvar_VariableValue(cvarName);

	if (Q_stricmp(cvarName, "s_khz") == 0)
	{
		trap->Cmd_ExecuteText( EXEC_APPEND, "snd_restart\n" );
		return;
	}

	if ( !Q_stricmp( cvarName, "ui_SetName" ) ) {
		char buf[MAX_NETNAME] = {0};
		Q_strncpyz( buf, ui_Name.string, sizeof( buf ) );
		trap->Cvar_Set( "name", buf );
	}
	else if (Q_stricmp(cvarName, "ui_setRate") == 0) {
		if (rate.value >= 5000) {
			trap->Cvar_Set("cl_maxpackets", "30");
			trap->Cvar_Set("cl_packetdup", "1");
		} else if (rate.value >= 4000) {
	...
	else if ( !Q_stricmp( cvarName, "ui_GetName" ) ) {
		char buf[MAX_NETNAME] = {0};
		Q_strncpyz( buf, name.string, sizeof( buf ) );
		trap->Cvar_Set( "ui_Name", buf );
	}
	else if (Q_stricmp(cvarName, "ui_r_colorbits") == 0)
	{
		switch (val)
		{
			case 0:
	...
	else if (Q_stricmp(cvarName, "ui_r_lodbias") == 0)
	{
		switch (val)
		{
			case 0:
	...
	else if (Q_stricmp(cvarName, "ui_r_glCustom") == 0)
	{
		switch (val)
		{
		case 0:	// high quality
	...
	else if (Q_stricmp(cvarName, "ui_mousePitch") == 0)
	{
		if (val == 0)	trap->Cvar_SetValue( "m_pitch", 0.022f );
		else			trap->Cvar_SetValue( "m_pitch", -0.022f );
	}
	else if (Q_stricmp(cvarName, "ui_mousePitchVeh") == 0)
	{
...
static bool UI_DeferMenuScript ( char **args )
{
	const char* name;
	...
	if ( !Q_stricmp ( name, "VideoSetup" ) )
	{
		...
		deferred = !!ui_r_modified.integer;
	...
	else if ( !Q_stricmp ( name, "RulesBackout" ) )
	{
		...
		deferred = !!ui_rules_backout.integer;
...
// This is to create a interface which allows you to discard your changes if you did something you didnt want
...
static void UI_SetBotButton ( void )
{
	...
	server = sv_running.value;

	// If in siege or a client, don't show add bot button
	if ( server == 0 )	// If it's not siege, don't worry about it
	{
		...
		if (item)
		{
			Menu_ShowItemByName(menu, name, false);
		}
	}
}
...
static void UI_UpdateSaberHilt( bool secondSaber )
{
	menuDef_t *menu;
	itemDef_t *item;
	char model[MAX_QPATH];
...
static void UI_UpdateSaberColor( bool secondSaber )
{
}
...
static void UI_UpdateCharacter( bool changedModel )
{
	menuDef_t *menu;
	itemDef_t *item;
	char modelPath[MAX_QPATH];
...
static bool UI_CheckPassword( void )
{
...
void UI_ClampMaxPlayers( void ) {
	...
	// power duel requires 3 players
		...
		if ( (int)sv_maxClients.value < 3 )
			trap->Cvar_Set( "sv_maxClients", "3" );
	}

	// can never exceed MAX_CLIENTS
	if ( (int)sv_maxClients.value > MAX_CLIENTS ) {
...
static void UI_RunMenuScript(char **args)
{
	...
	if (String_Parse(args, &name))
	{
		if (Q_stricmp(name, "StartServer") == 0)
		{
			...
			//trap->Cvar_Set("g_redTeam", ui_teamName.string);
			//trap->Cvar_Set("g_blueTeam", ui_opponentName.string);
			...
			warmupTime = g_warmup.value;
			doWarmup = g_doWarmup.value;
			...
			if (g_gametype.value == GT_DUEL ||
				g_gametype.value == GT_POWERDUEL)
		...
		else if (Q_stricmp(name, "createFavorite") == 0)
		{
			...
			//rww - don't know why this check was here.. why would you want to only add new favorites when the filter was favorites?
			{
				...
				name[0] = addr[0] = '\0';
				Q_strncpyz(name, 	ui_favoriteName.string, sizeof( name ) );
				Q_strncpyz(addr, 	ui_favoriteAddress.string, sizeof( addr ) );
				if (/*strlen(name) > 0 &&*/ strlen(addr) > 0) {
					res = trap->LAN_AddServer(AS_FAVORITES, name, addr);
					if (res == 0) {
						// server already in the list
			...
			if (String_Parse(args, &orders)) {
				int selectedPlayer = cg_selectedPlayer.value;
		...
		{
			UI_UpdateCharacter( false );
		}
		else if (Q_stricmp(name, "characterchanged") == 0)
		{
			UI_UpdateCharacter( true );
		}
		...
		{
			int forcePowerDisable = g_forcePowerDisable.value;
		...
		else if (Q_stricmp(name, "dualForcePowers") == 0)
		{
			...
			dualforcePower = ui_dualforcepower.value;
		...
		else if (Q_stricmp(name, "forcePowersDisable") == 0)
		{
			...
			forcePowerDisable = g_forcePowerDisable.value;
		...
		else if (Q_stricmp(name, "weaponDisable") == 0)
		{
			...
			{
				weaponDisable = g_duelWeaponDisable.integer;
			}
			else
			{
				weaponDisable = g_weaponDisable.integer;
			}
		...
		{
			trap->Cvar_Set("ui_mousePitch", (m_pitch.value >= 0) ? "0" : "1");
		}
...
// Return false if the infostring contains nonprinting characters, or if the hostname is blank/undefined
...
//		visible = true;
		...
		if (ping > 0 || ui_netSource.integer == UIAS_FAVORITES) {
			...
			if ( ui_browserFilterInvalidInfo.integer != 0 && !UI_ServerInfoIsValid( info ) ) {
				trap->LAN_MarkServerVisible( lanSource, i, false );
				continue;
			}
			...
			if (uiInfo.gameTypes[ui_gametype.integer].gtEnum != -1) {
				game = atoi(Info_ValueForKey(info, "gametype"));
				if (game != uiInfo.gameTypes[ui_gametype.integer].gtEnum) {
					trap->LAN_MarkServerVisible(lanSource, i, false);
					continue;
				}
			}
			...
			switch (column) {
				case SORT_HOST :
					if (ping <= 0) {
						...
						{
							bool saberOnly = true;
							bool restrictedForce = false;
							bool allForceDisabled = false;
							int wDisable, i = 0;
...
static qhandle_t UI_FeederItemImage(float feederID, int index) {
	...
	else if (feederID == FEEDER_Q3HEADS)
	{
		...
		{ //we want it to load them as it draws them, like the TA feeder
			...
			int selModel = ui_selectedModelIndex.value;
...
bool UI_FeederSelection(float feederFloat, int index, itemDef_t *item)
{
	static char info[MAX_STRING_CHARS];
...
static bool GameType_Parse(char **p) {
	char *token;
	...
	while ( 1 ) {
		...
		if (token[0] == '{') {
			...
			if (uiInfo.numGameTypes < MAX_GAMETYPES) {
				...
				Com_Printf("Too many game types, last one replaced!\n");
			}
	...
	while ( 1 ) {
		...
		if (token[0] == '{') {
			...
			//if (uiInfo.mapCount == 0) {
  			...
  			//  uiInfo.mapList[uiInfo.mapCount].cinematic = trap->CIN_PlayCinematic(va("%s.roq",uiInfo.mapList[uiInfo.mapCount].mapLoadName), false, false, true, 0, 0, 0, 0);
			//}
...
nextSearch:
	...
	for (j=0; j<numfiles && uiInfo.forceConfigCount < MAX_FORCE_CONFIGS;j++,fileptr+=filelen+1)
	{
		...
		if (lightSearch)
		{
			uiInfo.forceConfigSide[uiInfo.forceConfigCount] = true; //light side config
		}
		else
		{
			uiInfo.forceConfigSide[uiInfo.forceConfigCount] = false; //dark side config
		}
...
static bool bIsImageFile(const char* dirptr, const char* skinname)
{
...
//static bool UI_ParseColorData(char* buf, playerSpeciesInfo_t &species)
static bool UI_ParseColorData(char* buf, playerSpeciesInfo_t *species,char*	file)
{
	const char	*token;
	const char	*p;
	...
	while ( p )
	{
		...
		token = COM_ParseExt( &p, true );	//looking for action block {
		if ( token[0] != '{' )
		{
			return false;
		}

		token = COM_ParseExt( &p, true );	//looking for action commands
		while (token[0] != '}')
		{
			if ( token[0] == 0)
			{	//EOF
			...
			token = COM_ParseExt( &p, true );	//looking for action commands or final }
		}
...
static void UI_FreeAllSpecies( void )
{
	int i;
...
static void UI_BuildPlayerModel_List( bool inGameLoad )
{
...
void UI_Init( bool inGameLoad ) {
	...
	if ( inGameLoad ) {
		UI_LoadMenus( MENUSET_INGAME, true );
	}
	...
	uiInfo.currentCrosshair = (int)cg_drawCrosshair.value;
	trap->Cvar_Set("ui_mousePitch", (m_pitch.value >= 0) ? "0" : "1");
	trap->Cvar_Set("ui_mousePitchVeh", (m_pitchVeh.value >= 0) ? "0" : "1");
...
void UI_Refresh( int realtime )
{
	...
	if (parsedFPMessage[0] && FPMessageTime > realtime)
	{
		...
		font.Paint(10, 0, 1, txtCol, parsedFPMessage, 0, 1024, txtStyle, FONT_MEDIUM);
	}
	*/
...
static void UI_DisplayDownloadInfo( const char *downloadName, float centerPoint, float yStart, float scale, int iMenuFont) {
	...
	downloadSize = cl_downloadSize.value;
	downloadCount = cl_downloadCount.value;
	downloadTime = cl_downloadTime.value;

	leftWidth = 320;

	UI_SetColor(colorWhite);

	Font font( iMenuFont, scale, ITEM_ALIGN_CENTER );
	font.Paint(centerPoint, yStart + 112, sDownLoading, colorWhite );
	font.Paint(centerPoint, yStart + 192, sEstimatedTimeLeft, colorWhite );
	font.Paint(centerPoint, yStart + 248, sTransferRate, colorWhite );
	...
	font.Paint(centerPoint, yStart+136, s, colorWhite);

	UI_ReadableSize( dlSizeBuf,		sizeof dlSizeBuf,		downloadCount );
	UI_ReadableSize( totalSizeBuf,	sizeof totalSizeBuf,	downloadSize );

	if (downloadCount < 4096 || !downloadTime) {
		font.Paint(leftWidth, yStart+216, "estimating", colorWhite);
		font.Paint(leftWidth, yStart+160, va("(%s %s %s %s)", dlSizeBuf, sOf, totalSizeBuf, sCopied), colorWhite);
	} else {
		...
		if (downloadSize && xferRate) {
			...
			font.Paint(leftWidth, yStart+216, dlTimeBuf, colorWhite);
			font.Paint(leftWidth, yStart+160, va("(%s %s %s %s)", dlSizeBuf, sOf, totalSizeBuf, sCopied), colorWhite);
		} else {
			font.Paint(leftWidth, yStart+216, "estimating", colorWhite);
			if (downloadSize) {
				...
				font.Paint(leftWidth, yStart+160, va("(%s %s)", dlSizeBuf, sCopied), colorWhite);
			}
		}

		if (xferRate) {
			font.Paint(leftWidth, yStart+272, va("%s/%s", xferRateBuf,sSec), colorWhite);
		}
	}
}
...
void UI_DrawConnectScreen( bool overlay ) {
	...
	Font font( FONT_MEDIUM, scale, ITEM_ALIGN_CENTER );
	info[0] = '\0';
	if( trap->GetConfigString( CS_SERVERINFO, info, sizeof(info) ) ) {
		...
		font.Paint(centerPoint, yStart, va( /*"Loading %s"*/sStringEdTemp, Info_ValueForKey( info, "mapname" )), colorWhite);
	}

	if (!Q_stricmp(cstate.servername,"localhost")) {
		trap->SE_GetStringTextString("MENUS_STARTING_UP", sStringEdTemp, sizeof(sStringEdTemp));
		font.Paint(centerPoint, yStart + 48, sStringEdTemp, colorWhite, ITEM_TEXTSTYLE_SHADOWEDMORE);
	} else {
		...
		font.Paint(centerPoint, yStart + 48, text, colorWhite, ITEM_TEXTSTYLE_SHADOWEDMORE);
	}

	// display global MOTD at bottom
	font.Paint(centerPoint, 425, Info_ValueForKey( cstate.updateInfoString, "motd" ), colorWhite);
	// print any server info (server full, bad version, etc)
	if ( cstate.connState < CA_CONNECTED ) {
		font.Paint(centerPoint, yStart + 176, cstate.messageString, colorWhite);
	}
	...
	if (Q_stricmp(cstate.servername,"localhost")) {
		font.Paint(centerPoint, yStart + 80, s, colorWhite);
	}
