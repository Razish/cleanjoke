...
static inline bool CM_PlaneFromPoints( vec4_t plane, vec3_t a, vec3_t b, vec3_t c ) {
	vec3_t	d1, d2;
...
// Adds columns as necessary to the grid until all the aproximating points are within SUBDIVIDE_DISTANCE from the true
//	curve
...
static inline bool CM_ValidateFacet( facet_t *facet ) {
	float		plane[4];
	int			j;
	winding_t	*w;
	vec3_t		bounds[2];
...
static inline void CM_PatchCollideFromGrid( cGrid_t *grid, patchCollide_t *pf ) {
	...
	int numFacets;
	facets = (facet_t*) Z_Malloc(MAX_FACETS*sizeof(facet_t), TAG_TEMP_WORKSPACE, false, 4);
